...
CFDI_TEMPLATE = 'l10n_mx_edi_cfdi_33.payment10'
CFDI_XSLT_CADENA = 'l10n_mx_edi_cfdi_33/data/3.3/cadenaoriginal.xslt'
CFDI_SAT_QR_STATE = {
...
class AccountPayment(models.Model):
    ...
    def l10n_mx_edi_is_required(self):
        ...
        if not sum(self.invoice_ids.mapped('payment_term_id').mapped(
                'line_ids').mapped('days')) > 0:
            return False
        return required
    ...
    def l10n_mx_edi_payment_data(self):
        ...
        rate = ('%.6f' % (
            self.currency_id.with_context(date=self.payment_date).compute(
                1, mxn, False))) if self.currency_id.name != 'MXN' else False
        return {
            'mxn': mxn,
            'payment_date': date,
            'payment_rate': rate,
