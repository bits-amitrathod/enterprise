...
class purchase_history(models.TransientModel):
    ...
    def get_report_values(self):
        ...
        _logger.info('AKASH %r', pId)
        groupby_dict = {}
        filtered_by_date=purchase_orders
        ...
        for user in groupby_dict.keys():
            ...
            for order in groupby_dict[user]:
                ...
                # temp_2.append(datetime.datetime.strptime(str(query_result['min']), '%Y-%m-%d %H:%M:%S.%f').date().strftime('%d-%m-%Y'))
                # temp_2.append(datetime.datetime.strptime(str(query_result['max']), '%Y-%m-%d %H:%M:%S.%f').date().strftime('%d-%m-%Y'))
                temp.append(temp_2)
