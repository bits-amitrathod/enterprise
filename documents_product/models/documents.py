...
class IrAttachment(models.Model):
    ...
    def _set_folder_product(self, vals):
        if not vals.get('folder_id'):
            if self.env.user.company_id.dms_product_settings:
    ...
    def create(self, vals_list):
        for vals_dict in vals_list:
            if vals_dict.get('res_model') in ('product.template', 'product.product'):
                vals_dict.update(self._set_folder_product(vals_dict))
        return super(IrAttachment, self).create(vals_list)

    def write(self, vals):
        if vals.get('res_model') in ('product.template', 'product.product'):
            vals = self._set_folder_product(vals)
